...
/**
 * This class is in charge of creating Jimple code for a given statement (and
 * its nested statements, if applicable).
 */
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		/*
		 * TODO: generate code for break statement (hint: use ASTNode.getEnclosingLoop
		 * and breakTargets; use units.add() to insert the statement into the
		 * surrounding method)
		 */
	...
	public Void visitWhileStmt(WhileStmt nd) {
		/*
		 * TODO: generate code for while statement as discussed in lecture; add the NOP
		 * statement you generate as the break target to the breakTargets map
		 */
		NopStmt labelBegin = j.newNopStmt();
		NopStmt labelBreak = j.newNopStmt();

		breakTargets.put(nd, labelBreak);

		units.add(labelBegin);
		Value cond = ExprCodeGenerator.generate(nd.getExpr(), fcg);
		units.add(j.newIfStmt(j.newEqExpr(cond, IntConstant.v(0)), labelBreak));
		nd.getBody().accept(this);
		units.add(j.newGotoStmt(labelBegin));
		units.add(labelBreak);
