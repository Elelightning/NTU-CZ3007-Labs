...
/**
 * This class is in charge of creating Jimple code for a given expression (and
 * its nested expressions, if applicable).
 */
public class ExprCodeGenerator extends Visitor<Value> {
	...
	/**
	 * Ensures that the given value can be used as an operand; that is, if the value
	 * is not a {@link Local} or a {@link Constant}, this method allocates a new
	 * temporary variable and stores the value into that temporary.
	 */
	...
	public Value visitAssignment(Assignment nd) {
		...
		Value lhs = nd.getLHS().accept(this), rhs = wrap(nd.getRHS().accept(this));
	...
	public Value visitArrayLiteral(ArrayLiteral nd) {
		...
		for (int i = 0; i < nd.getNumElement(); ++i) {
			// generate code to store the individual expressions into the elements of the
			// array
	...
	public Value visitVarName(VarName nd) {
		...
		if (decl.isLocal()) {
			...
			SootClass declaringClass = fcg.getModuleCodeGenerator().getProgramCodeGenerator()
					.getSootClass(decl.getModule());
			Type fieldType = SootTypeUtil.getSootType(decl.getTypeName().getDescriptor());
			return Jimple.v()
					.newStaticFieldRef(Scene.v().makeFieldRef(declaringClass, decl.getName(), fieldType, true));
		}
	}
	...
	public Value visitBinaryExpr(BinaryExpr nd) {
		 ...
		 * to determine the type of binary expression you are dealing with, or generate
		 * code in the more specialised visitor methods visitAddExpr, visitSubExpr,
		 * etc., instead
		 */
